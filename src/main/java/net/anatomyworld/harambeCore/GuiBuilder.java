...
public class GuiBuilder {
    ...
    public Map<Integer, Double>  getSlotCosts(String guiKey)                    { return guiSlotCosts.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, Boolean> getCostPerStack(String guiKey)                 { return guiCostPerStack.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, String>  getAcceptedItems(String guiKey)                { return guiAcceptedItems.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, Integer> getAcceptedAmounts(String guiKey)              { return guiAcceptedAmounts.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, String>  getOutputItems(String guiKey)                  { return guiOutputItems.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, Integer> getPayoutAmounts(String guiKey)                { return guiPayoutAmounts.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, InputActionType> getInputActions(String guiKey)         { return guiInputActions.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, List<Integer>>   getSlotConnections(String guiKey)      { return guiSlotConnections.getOrDefault(guiKey, Collections.emptyMap()); }
    ...
    public Map<Integer, String>          getCheckItems(String guiKey)           { return guiCheckItems.getOrDefault(guiKey, Collections.emptyMap()); }
    public Map<Integer, String>          getRewardGroups(String guiKey)         { return guiRewardGroups.getOrDefault(guiKey, Collections.emptyMap()); }
    ...
    private Inventory generateGui(String guiKey) {
            ...
            for (String buttonKey : buttonsSection.getKeys(false)) {
                ...
                switch (slotType) {
                    ...
                    case BUTTON, CHECK_BUTTON -> {
                        ...
                        String outItem   = null;
                        int    payAmt    = 1;
                        Object outCfg    = bc.get("output_item");
                        if (outCfg instanceof ConfigurationSection oc) {
                        ...
                        } else if (outCfg instanceof String s) {
                        ...
                        ItemStack btnItem = new ItemStack(Material.STONE);
                        if (d != null) {
                            if (d.contains("mythic")) {
                                String mythicId = d.getString("mythic");
                                ItemStack myth = itemRegistry.getItem(mythicId);
                                if (myth != null) btnItem = myth;
                            } else {
                                String matName = d.getString("material", "STONE");
                                String itemName = d.getString("name", "&fButton");
                                int cmd = d.getInt("custom_model_data", 0);
        ...
        slotConnections.forEach((checkBtn, inputs) -> inputs.forEach(in -> reverse.put(in, checkBtn)));
    ...
    public void handleButtonClick(Player player, String guiKey, int slot) {
        ...
        if (logics.containsKey(slot)) {
            ...
            if (raw.startsWith("console:")) { sender = Bukkit.getConsoleSender(); raw = raw.substring(8); }
            else if (raw.startsWith("player:")) { sender = player; raw = raw.substring(7); }
            else sender = player;
    ...
    public String getGuiKeyByInventory(Player player, Inventory inv) {
        ...
        return map.entrySet().stream().filter(e -> e.getValue().equals(inv))
